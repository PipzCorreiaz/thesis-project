...
The following two subsections will detail how Monte Carlo Methods and Belief Distributions can be applied in hidden information games.
...
The value of a game tree node only considers its children' values.
...
%%% IIMC
Recently, Furtak and Buro \cite{Furtak} presented a new search algorithm called \gls{iimc} that can be suitably applied to hidden information games and reduces the \emph{strategy fusion} problem.
...
However, \gls{ismcts} and \gls{iimc} add computational burden when compared to \gls{pimc}.
...
\hline
\gls{pimc}      & \begin{tabular}[c]{@{}l@{}}Computation can be offline\\ Easy to parallel\end{tabular}                                             & \begin{tabular}[c]{@{}l@{}}Strategy fusion\\ Non-locality\end{tabular}                                        \\ \hline
\gls{ismcts}    & \begin{tabular}[c]{@{}l@{}}Computation can be offline\\ Easy to parallel\\ Computational budget\end{tabular}                      & \begin{tabular}[c]{@{}l@{}}Strategy fusion (less than \gls{pimc})\\ Non-locality\\ Complexity\end{tabular}                       \\ \hline
\gls{iimc}      & \begin{tabular}[c]{@{}l@{}}Computation can be offline\\ Easy to parallel\\ Allow a different player model per player\end{tabular} & \begin{tabular}[c]{@{}l@{}}Strategy fusion (less than \gls{pimc})\\ Non-locality\\ Complexity\end{tabular} \\ \hline
\end{tabular}
}
...
\begin{table}[h]
\caption{Game state inference and opponent modelling techniques. Techniques signed with \ding{53}, \ding{51} and $\sim$ symbols are, respectively, not suitable, suitable and conditionally suitable to the \emph{Sueca} domain.}
\resizebox{\textwidth}{!}{%
